...
const userReducer = (state: User = { loading: true }, action: UserAction) => {
  switch (action.type) {
    case LOGIN_USER:
      var { id, whitelist, username, isAdmin, isModerator, steam, discord } = action.payload;
      whitelist === 0 ? (whitelist = false) : (whitelist = true);
      return { ...state, id, username, whitelist, isAdmin, isModerator, steam, discord, loading: false };
    case LOGOFF_USER:
